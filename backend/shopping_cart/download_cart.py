...
def download_ingredients(user):
    ...
    ingredient_recipes = IngredientRecipe.objects.filter(recipe__recipe_cart__user=user)
    for ingredient_recipe in ingredient_recipes:
        name = ingredient_recipe.ingredient.name
        mesurement_unit = ingredient_recipe.ingredient.measurement_unit
        ingredient = f'{name}, {mesurement_unit}'
